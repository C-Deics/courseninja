...
app.get("/autocomplete", function(req, res) {
  ...
  if (req.query.search == "program") {
    ...
    var sql =
      "SELECT schoolId FROM schools WHERE schoolName = '" + school + "';";
    con.query(sql, function(err, results) {
      if (err) {
        ...
        var sql =
          "SELECT programName FROM programs WHERE schoolId = '" +
          results[0].schoolId +
          "';";
        con.query(sql, function(err, results) {
...
app.post("/flag/review/:id", function(req, res) {
  ...
  con.query(sql, function(err, results) {
    if (err) {
      ...
      var sql =
        "INSERT INTO flags (objectType, objectReference, objectBody, reason, isComplete) VALUES ('review', '" +
        req.params.id +
        "', '" +
        review.reviewBody +
        "', '" +
...
app.post("/flag/program/:id", function(req, res) {
  var sql =
    "INSERT INTO flags (objectType, objectReference, objectBody, reason, isComplete) VALUES ('program', '" +
    req.params.id +
    "', '" +
    req.body.selection +
    "', '" +
